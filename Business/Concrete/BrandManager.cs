...
namespace Business.Concrete
{
    public class BrandManager:IBrandService
    {
        ...
        {
            return new SuccessDataResult<Brand>(_brandDal.Get(x => x.BrandId == entity.BrandId));
        }
        ...
        public IResult GetList(List<Brand> list)
        {
            ...
            {
                Console.WriteLine("{0}- Brand Name: {1}", brand.BrandId, brand.Name);
            }
        ...
        {
            Brand b = new Brand();
            if (_brandDal.GetAll().Any(x => x.BrandId == Id))
            {
                b = _brandDal.GetAll().FirstOrDefault(x => x.BrandId == Id);
            }
